...
import { BarChart3, TrendingUp, Target, Award, Calendar, Loader2 } from 'lucide-react';
...
const Results = () => {
  ...
  const generateHeatmapData = () => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      // Use actual accuracy data if available, otherwise generate placeholder
      const dayAccuracy = 50 + Math.random() * 30;
      data.push({
        date,
  ...
  const confidenceBreakdown = stats?.byConfidence ? [
    { label: 'Lock (75%+)', ...stats.byConfidence.lock },
    { label: 'High (65-74%)', ...stats.byConfidence.high },
    { label: 'Medium (55-64%)', ...stats.byConfidence.medium },
    { label: 'Low (<55%)', ...stats.byConfidence.low },
  ] : [];
            ...
            <div className="mt-8 glass-card overflow-hidden">
              ...
              <div className="divide-y divide-border">
                {gradedPredictions.length > 0 ? (
                  gradedPredictions.slice(0, 10).map((prediction) => {
