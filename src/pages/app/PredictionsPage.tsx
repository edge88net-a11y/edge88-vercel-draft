  ...
  return (
    <div>
      ...
      {isLoading && !isMaintenanceMode ? (
        ...
        {filteredAndSortedPredictions.map((prediction, index) => (
            <div key={prediction.id} className="relative">
              {shouldLockPrediction(index) ? (
                <SubscriptionGate isLocked={true}>
      ...
      {!isPro && filteredAndSortedPredictions.length > (user ? FREE_PICKS_LIMIT * 2 : FREE_PICKS_LIMIT) && (
        <div className="mt-8 glass-card p-6 text-center bg-gradient-to-r from-primary/10 via-accent/10 to-primary/10 border-primary/30">
          <h3 className="text-xl font-bold mb-2">
            {t.upgradeToSeeAll || 'Unlock All Picks'}
          </h3>
