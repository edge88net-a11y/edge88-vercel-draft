...
import { ChevronDown, TrendingUp, Lock, AlertTriangle, ThermometerSun, DollarSign, MessageCircle, Users, History } from 'lucide-react';
    ...
    >
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex flex-col items-start gap-2">
          <div className={cn(
            'flex items-center gap-3 rounded-lg px-2 py-1 -ml-2',
        ...
        <div className="mt-4 animate-fade-in border-t border-border pt-4 space-y-4">
                ...
                {prediction.keyFactors.sentiment && (
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <span className="text-muted-foreground ml-1">
                        Public: {prediction.keyFactors.sentiment.public}% | Sharp: {prediction.keyFactors.sentiment.sharp}%
                      </span>
                    </div>
                  </div>
                )}
          ...
          <div>
            ...
            <div className="space-y-2">
              <BreakdownBar label="Research & Analysis" value={breakdown.research} color="bg-primary" />
              <BreakdownBar label="Odds & Market" value={breakdown.odds} color="bg-accent" />
              <BreakdownBar label="Historical Data" value={breakdown.historical} color="bg-success" />
              {breakdown.sentiment && (
                <BreakdownBar label="Sentiment" value={breakdown.sentiment} color="bg-yellow-400" />
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {bookmakerOdds.slice(0, 3).map((bk, idx) => (
                <div 
                  key={bk.bookmaker} 
                  className={cn(
                ...
                >
                  <p className={cn('text-xs', idx === bestOddsIndex ? 'text-success' : 'text-muted-foreground')}>
                    {idx === bestOddsIndex ? 'Best' : bk.bookmaker}
                  </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-muted/50 p-3">
              ...
              <p className="text-sm font-medium">{prediction.dataSources || 12} {t.verifiedSources}</p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3">
              <p className="text-xs text-muted-foreground">{t.modelVersion}</p>
              <p className="text-sm font-medium">{prediction.modelVersion || 'Edge88 v3.2'}</p>
            </div>
          </div>
        </div>
      )}
