    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-xl md:hidden safe-area-bottom">
      <div className="flex items-center justify-around px-2 py-2">
        {navItems.map((item) => {
          ...
          const targetPath = !user && item.path === '/dashboard' ? '/login' : item.path;
        ...
        {/* Profile/Login tab */}
        <Link
          to={user ? '/saved-picks' : '/login'}
          className={cn(
            'flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all min-w-[60px]',
            (location.pathname === '/saved-picks' || location.pathname === '/login' || location.pathname === '/signup')
              ? 'text-primary bg-primary/10'
              : 'text-muted-foreground hover:text-foreground'
          )}
        >
          <div className={cn(
            'relative',
            (location.pathname === '/saved-picks' || location.pathname === '/login') && 
          ...
          <span className="text-[10px] font-medium">{user ? (t.profile || 'Profile') : 'Login'}</span>
        </Link>
      </div>
    </nav>
  );
